...
"""
-------------------------------------------------
   File Name：     relation_search_view.py
   Description :  实体查询及关系查询实现
   Author :       charl
   date：          2018/11/28
-------------------------------------------------
	...
	if(request.GET):
		...
		if(len(entity1) != 0 and len(relation) == 0 and len(entity2) == 0):
			...
			if(len(searchResult)>0):
				return render(request,'relation_search.html',{'searchResult':json.dumps(searchResult,ensure_ascii=False)})
