...
class Neo4j():
	...
	def matchItembyTitle(self,value):
		...
		answer = matcher.match(label="Item",property_key="title",property_value=value)
		return answer

	# 根据title值返回互动百科item
	def matchHudongItembyTitle(self,value):
		matcher = NodeMatcher(self.graph)
		answer = matcher.match(label="HudongItem",property_key="title",property_value=value)
		return answer

	# 根据entity的名称返回关系
	def getEntityRelationbyEntity(self,value):
		matcher = NodeMatcher(self.graph)
		answer = matcher.get("MATCH (entity1) - [rel] -> (entity2)  WHERE entity1.title = \"" +value +"\" RETURN rel,entity2")
		return answer
