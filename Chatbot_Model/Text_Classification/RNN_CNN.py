...
from Preprocessor import read_vocab, read_category, batch_iter, process_file, build_vocab
...
def evaluate(sess, x_, y_):
    ...
    '''
    data_len = len(x_)
    batch_eval = batch_iter(x_, y_, 120)
    total_loss = 0.0
    total_acc = 0.0
...
def train():
    ...
    # 配置Tensorboard，重新训练时，要将tensorboard文件夹删除，不然图会被覆盖
    tensorboard_dir = 'tensotboard/textcnn'
    ...
    require_improvement = 1000   # 如果超过1000轮未提升，则结束训练
...
if __name__ == '__main__':
    ...
    config = TCNNConfig()
    if not os.path.exists(vocab_dir):    # 如果没有词汇表，则重建
