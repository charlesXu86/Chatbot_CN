...
def get_add(judge_res):
    pattern1 = re.compile(
        r'.*(坐落于|坐落在|位于|名下的?|所有的?)(.*?)(幢?|栋?|室?|号?|层?)(住房|住宅|营业场所|写字楼|商品房|房地产|的?不动产|使用权|土地|公寓|办公楼?|门面房?|商铺|楼房|仓库|工业用地|工业厂房|承包地|商住楼|车库|车位|亩).*?')
    match_addr1 = re.search(pattern1, judge_res)
    match_addr2 = re.search(r'(坐落于|坐落在|位于|名下的?|所有的?)(.*?室)',judge_res)
    match_addr3 = re.search(r'(坐落于|坐落在|位于|名下的?|所有的?)(.*?号)',judge_res)
    match_addr4 = re.search(r'(坐落于|坐落在|位于|名下的?|所有的?)(.*?房)',judge_res)
    match_addr5 = re.search(r'(坐落于|坐落在|位于|名下的?|所有的?)(.*?栋)',judge_res)
    match_addr6 = re.search(r'(坐落于|坐落在|位于|名下的?|所有的?)(.*?店)',judge_res)
    if (match_addr1):
        return match_addr1.group(2)
    if (match_addr2):
        return match_addr2.group(2)
    if (match_addr3):
        return match_addr3.group(2)
    if (match_addr4):
        return match_addr4.group(2)
    if (match_addr5):
        return match_addr5.group(2)
    if (match_addr6):
...
if __name__ == '__main__':
    ...
    str =  "将原属被执行人黄华润所有的坐落于福州市晋安区岳峰镇连江北路116号二化新村三区5#楼301单元作价1100813元时起转移；"
           # "江东路136号阳光城世纪广场办公楼B#楼18层13办公（YR1308058）及坐落于福州市马尾区马尾镇儒江东路136号阳"
