...
def main(_):
    ...
    train_word = np.load(data_path + 'train_word.npy')
    train_pos1 = np.load(data_path + 'train_pos1.npy')
    train_pos2 = np.load(data_path + 'train_pos2.npy')

    settings = re_network.Settings()
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            with tf.variable_scope("model", reuse=None, initializer=initializer):
                m = re_network.GRU(is_training=True, word_embeddings=wordembedding, settings=settings)
